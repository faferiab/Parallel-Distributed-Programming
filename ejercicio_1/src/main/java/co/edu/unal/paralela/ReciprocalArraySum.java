...
public final class ReciprocalArraySum {
    ...
    private static class ReciprocalArraySumTask extends RecursiveAction {
        ...
        protected void compute() {
            if (endIndexExclusive - startIndexInclusive >= 1_000) {
    ...
    protected static double parArraySum(final double[] input) {
        ...
        ReciprocalArraySumTask t = new ReciprocalArraySumTask(getChunkStartInclusive(0, 2, input.length),
                getChunkEndExclusive(0, 2, input.length), input);
        ReciprocalArraySumTask u = new ReciprocalArraySumTask(getChunkStartInclusive(1, 2, input.length),
                getChunkEndExclusive(1, 2, input.length), input);
        ForkJoinPool tp = new ForkJoinPool(2);
    ...
    protected static double parManyTaskArraySum(final double[] input,
        ...
        ForkJoinPool tp = new ForkJoinPool(numTasks);
        List<ReciprocalArraySumTask> tasks = IntStream.range(0, numTasks)
                .mapToObj(i -> new ReciprocalArraySumTask(getChunkStartInclusive(i, numTasks, input.length),
                        getChunkEndExclusive(i, numTasks, input.length), input))
                .toList();
        tasks
                .stream()
                .forEach(ReciprocalArraySumTask::fork);
